...
static uint8_t myrand() { return (uint8_t)(std::rand() & 0xFF); }

static void mysrand(unsigned int seed) { std::srand(seed); }
...
BOOST_AUTO_TEST_CASE(write) {
  ...
  for (int i = 0; i < Len; ++i) {
    ...
    BOOST_CHECK_EQUAL(
        Fif.Tail,
        (void *)&((uint8_t *)FifBuff)[i + 1 >= Len ? i + 1 - Len : i + 1]);
...
BOOST_AUTO_TEST_CASE(read) {
  ...
  for (int i = 0; i < Len; ++i) {
    ...
    BOOST_CHECK_EQUAL(Fif.Head,
                      (void *)&FifBuff[i + 1 >= Len ? i + 1 - Len : i + 1]);
    BOOST_CHECK_EQUAL(Fif.Tail, (void *)&FifBuff[0]);
    BOOST_CHECK_EQUAL(FifBuff[i], x);
  }
